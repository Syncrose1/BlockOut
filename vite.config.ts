...
export default defineConfig(({ command }) => ({
  ...
  //
  // NOTE: NODE_ENV=production is set by the `build` npm script (not here) so it
  // is in the environment BEFORE Vite/@vitejs/plugin-react resolve dev-vs-prod.
  // Setting it inside this config factory runs too late — Vite ships the dev
  // React build. Keep it on the script.
